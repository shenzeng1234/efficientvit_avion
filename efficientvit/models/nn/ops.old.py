...
class ConvLayer(nn.Module):
    def __init__(
        ...
        #self.conv.weight = nn.Parameter(self.conv.weight.float())
...
class LiteMLA(nn.Module):
    ...
    def relu_linear_att(self, qkv: torch.Tensor) -> torch.Tensor:
        ...
        #AVION: no padding needed
        #v = F.pad(v, (0, 0, 0, 1), mode="constant", value=1)
        #print(f"*****size of trans_k = {trans_k.size()}, size of v = {v.size()}")
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        print(f"In LiteMLA.forward: x.device = {x.device}")
        qkv = self.qkv(x)
        print(f"self.qkv = {self.qkv}, x shape = {x.shape}, qkv shape = {qkv.shape}")
        multi_scale_qkv = [qkv]
        for op in self.aggreg:
            op.to("cuda:0")
            print(f"op = {type(op)}, op = {op}")
            multi_scale_qkv.append(op(qkv))
        multi_scale_qkv = torch.cat(multi_scale_qkv, dim=1)

        out = self.relu_linear_att(multi_scale_qkv)
        print(f"self.proj = {self.proj}, shape of out = {out.shape}")
        out = self.proj(out)
